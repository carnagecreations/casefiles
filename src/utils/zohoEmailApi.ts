...
export async function fetchInbox(settings: PricingSettings, limit = 20): Promise<InboxMessage[]> {
  const res = await fetch(workerUrl(settings, `/email/inbox?limit=${limit}`), {
...
export async function fetchMessageContent(settings: PricingSettings, id: string): Promise<string> {
